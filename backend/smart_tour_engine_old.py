...
class SmartTourEngine:
    def __init__(self):
        ...
        if self.gemini_key:
            ...
            self.model_name = 'gemini-3.5-flash' #免費版
        else:
            self.client = None
    ...
    def recommend_spots(self, user_need: str, city: str, tags: list, accumulated_spots: str) -> str:
        ...
        try:
            ...
            # 🚀 格式化鐵律：用 1| 和 2| 規格化輸出，不允許任何多餘的口語廢話
            prompt = f"""
            你現在是台灣頂級智慧旅遊規劃師。
            ...
            【重要輸出格式規範 - 結構化鐵律】
            你接下來回傳的內容，每一行都必須是以下兩種格式之一，絕對不准包含任何其他前言、客套話、Markdown ### 大標題或結尾提示！
            
            1. 如果這一行是「可以被挑選的真實旅遊景點」，請嚴格使用此格式輸出（用 | 分隔）：
            1|景點名稱|用於Google地圖定位的完整縣市行政區與景點名|推薦理由與詳細介紹描述

            2. 如果這一行是「大區域大標題、行程前言、廢話、旅遊規劃師小叮嚀或總結文字」，請嚴格使用此格式輸出：
            2|這是一段前言或小叮嚀總結內容文字

            【正確輸出範例】：
            2|臺南放鬆約會名單：10大極致浪漫候選清單
            2|一、都會藝文區（牽手散步與復古摩登的優雅約會）
            1|奇美博物館|臺南市仁德區奇美博物館|純白歐式宮殿建築倒映在護城河畔，非常適合情侶同遊。
            1|神農街|臺南市中西區神農街|當夜幕低垂，點點暖黃燈火點亮了整條百年老街，老派浪漫滿分。
            2|💡 智慧規劃師的浪漫指南小筆記：建議下午三點後再前往海線景點。
    ...
    def generate_final_itinerary(self, accumulated_spots: str, user_need: str, city: str, transport: str = "自駕", start_location: str = "臺北市", start_time: str = "08:00") -> str:
        ...
        try:
            ...
            3. 嚴禁輸出 any <br> 標籤、|| 符號、或 Markdown 表格，請全部改用標準「多層次段落空行」換行。
    ...
    def modify_itinerary(self, current_itinerary: str, modification_demand: str) -> str:
        ...
        try:
            ...
            4. 嚴禁輸出 any <br> 標籤、|| 符號、或 Markdown 表格，請全部改用標準「多層次段落空行」換行。
            5. 時間段與景點名字必須各自獨立成一行。
